<x-admin.layout.app>
    ...
    <form
        class="relative mt-7"
        method="POST"
        action="{{ route("LaravelBlock2Pdf.update", ["element_id" => $element->id, "id" => $collection->id]) }}"
    ...
    >
        ...
        <x-admin.atoms.row>
            ...
            <input
                ...
                data-uic-original="/storage/{{ $collection->getMedia('laravel_block2_pdf_category_original')->first()->id }}/{{ $collection->getMedia('laravel_block2_pdf_category_original')->first()->file_name }}"
                data-uic-path="{{ $collection->getMedia('laravel_block2_pdf_category')->first()->getUrl() }}"
            />
        </x-admin.atoms.row>
